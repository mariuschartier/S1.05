...
function toggleSerie(indice){
  const blockSeries = document.querySelector("#episodes");
  const listeSerie = blockSeries.querySelectorAll("article");

  console.log(blockSeries);
  console.log(listeSerie);
  listeSerie.forEach(element => {
    if(element.classList.contains('containt')){
      element.classList.remove('containt');
    }
  });
  ...
  listeSerie.forEach(element => {
    if (indice == compteur) {
      element.classList.add('containt');
    }
    compteur = compteur + 1;
  });
}